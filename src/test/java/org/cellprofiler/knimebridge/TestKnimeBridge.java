...
public class TestKnimeBridge {
	...
	private ImgPlus<DoubleType> makeImgPlus(long [] dims) {
		...
		return new ImgPlus<DoubleType>(img);
	}
	...
	public void testRun() {
		...
		Future<Object> server = handlePipelineReq(mock, pipeline, channels);
		ImgPlus<DoubleType> foo = makeImgPlus(new long[] {20, 37});
		ImgPlus<DoubleType> bar = makeImgPlus(new long[] {31, 18});
		...
		server = mock.runOnServer(new RunWithSockets() {
			...
			public void run(Socket socket) {
				...
				if (axisName.equals(Axes.X.getLabel())) {
